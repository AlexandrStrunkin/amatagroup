...
?>
	...
	<?endforeach;?>
		<input type="text" name="sf_EMAIL" size="20" placeholder="<?=GetMessage('emaul')?>" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>" />
...
?>
	<form action="<?=$arResult["FORM_ACTION"]?>">
		...
		<?endforeach;?>
			<input type="text" name="sf_EMAIL" size="20" value="" placeholder="<?=GetMessage('emaul')?>" title="<?=GetMessage("subscr_form_email_title")?>" />
			<input type="submit" name="OK" value="<?=GetMessage("subscr_form_button")?>" />
	</form>
<?
$frame->end();
