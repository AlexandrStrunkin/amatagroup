...
<?endif;?>
<p class="authTitle"><?=GetMessage("SEND_RESUME")?></p>                                          
<div class="workWithUs">
    <div class="hiddenPopup <?if (strlen($arResult["MESSAGE"]) > 0) { 
        echo 'hasMessage';
    }?>">
        <div class="message">
        ...
        <?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>       
            <?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
                <?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>                 
                            ...
                            switch ($INPUT_TYPE):
                                case "USER_TYPE":
                                    ?><textarea placeholder=<?=GetMessage("TEXT_AREA_PLACEHOLDER")?> required id="textarea"></textarea><?
                                ...
                                case "T":
                                    ...
                                    ?>
                            <textarea required placeholder="<?=GetMessage("TEXT_AREA_PLACEHOLDER")?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea> 
                                    <?
                                    }
                                break;
                                ...
                                case "N":
                                    ...
                                    ?>                            
                                    <input class="nameInput <?if($propertyID == PHONE_INPUT_ID){ echo 'phoneInput';} elseif($propertyID == EMAIL_INPUT_ID){ echo 'emailInput';} elseif($propertyID == TEXT_INPUT_ID){ echo 'textInput';}?>" placeholder='<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><?endif?>' required type="<?if($propertyID == PHONE_INPUT_ID){ echo 'phone';} elseif($propertyID == EMAIL_INPUT_ID){ echo 'email';} else { echo 'text';}?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><?
                                ...
                                case "F":
                                    for ($i = 0; $i<$inputNum; $i++)
                                    {
                                        ...
                                        <input id="uploadFile" required type="file" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>"  name="PROPERTY_FILE_<?=$propertyID?>_<?=$arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] ? $arResult["ELEMENT_PROPERTIES"][$propertyID][$i]["VALUE_ID"] : $i?>" />
                                        <div id="input_file" class="clickable">
                                            <div class="uploadBarText"><?=GetMessage("RESUME_FILE")?></div>
                                            <div class="deleteFile" style="display:none"></div>                         
                                        </div>                              
                                        <?
                ...
                <?endif?>     
            <?endif?>   
                        <p class="description"><?=GetMessage("EMPTY_FIELDS")?></p>                
